...
line_track_coef = '0'			    #	keeps the coeficent to send to Arduino
line_track_coef_previous = '0'
...
def Get_Signal():
    ...
    while True:
        ...
        try:
            ...
            line_track_coef = signal_array[6]
        except:
...
def Send_Arduino():
    ...
    global line_track_coef_previous
    ...
    while True:
        ...
        if line_track_signal!= previous_mode_state and lock == False:
            ...
            lock = False
        if line_track_coef != line_track_coef_previous and lock == False:
            line_track_coef_previous = line_track_coef
            ser.write(b"".join([line_track_coef.encode(),b'\n']))
            answer = ser.readline()   #implement if need
            lock = False
...
def Send_Status():
    ...
    while close_send_state == False:
        if lock == False:
            ...
            ser.write(command)
            try:
                mess_arr = ser.readline().decode()
                mess = mess_arr.split('|')
                online_state = mess[0]
                power_state = mess[1]
                battery1_current_state = mess[2]
                battery2_current_state = mess[3]
                main_bat_state = mess[4]
                emerg_state = mess[5]
                sensores_state = mess[6]
            except:
